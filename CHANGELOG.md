...
- Proxy: `createFolder` now requires 2+ distinct target nodes and de-duplicates
...
- Proxy: unmatched routes are now logged server-side (a generic 404 in the
  browser is otherwise hard to tell apart from a stale/outdated running
  process), and connection-level fetch failures are normalized instead of
  potentially leaking internal error detail into an HTTP response.
...
since no version number existed before this one:
