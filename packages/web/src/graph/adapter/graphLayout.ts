...
import type { FolderNodeData } from '../nodes/FolderNode'
import { FOLDER_TYPE_STYLE, receivesFromPeers, type ArrowDirection } from '../../encoding/folderTypeStyle'
import { folderColorMap } from '../../encoding/folderColors'
import { cssColor } from '../../encoding/colors'
import type { Selection } from '../selection'

/** Pure layout/data functions split out of reactFlowAdapter.tsx: mixing these with its component exports breaks Fast Refresh. */
...
export function foldersGraph(cluster: ClusterModel, selection: Selection) {
 ...
 * stay countable.
 */
export function nodesGraph(cluster: ClusterModel, selection: Selection) {
  ...
  for (const { a, b, folderIds } of pairFolders.values()) {
    folderIds.forEach((folderId, index) => {
      ...
      // Share mode, per endpoint: asymmetric shares are expected (CLAUDE.md),
      // so each end's arrow/lock depends only on that end's own type.
      const shareA = cluster.shares.find((s) => s.folderId === folderId && s.deviceId === a)
      const shareB = cluster.shares.find((s) => s.folderId === folderId && s.deviceId === b)
      const lockAtSource = shareA?.type === 'receiveencrypted'
      const lockAtTarget = shareB?.type === 'receiveencrypted'
      ...
      edges.push({
        ...
        data: {
          ...
          arrowAtTarget: shareB ? receivesFromPeers(shareB.type) : false,
          lockAtSource,
          lockAtTarget,
          dashed: lockAtSource || lockAtTarget,
        },
        style: {
